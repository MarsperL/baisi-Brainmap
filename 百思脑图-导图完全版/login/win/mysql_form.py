...
from login.ui.mysql_form import Ui_Frame as ui_form
...
class mysql_form(ui_form, QFrame):
    def __init__(self):
        ...
        self.setupUi(self)
        icon = QtGui.QIcon()
        icon.addPixmap(QtGui.QPixmap(":/icon/icon/database.svg"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        ...
        # 按钮事件绑定
        self.close_pushButton.clicked.connect(self.close_event)#关闭
        self.min_pushButton.clicked.connect(self.showMinimized)#最小化
        self.max_pushbutton.clicked.connect(self.handle_maxmize)#最大化
    ...
    def change_user_pushButton_event(self):
        ...
        # 字段检查
        if user_id == "":
            QMessageBox.information(self, "修改用户", "请输入需要修改的用户id")
            return
        # 进行修改用户
