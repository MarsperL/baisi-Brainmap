...
class Context():
    ...
    def __init__(self):
        ...
        self.m_textFont = None
        self.m_m_textFont = None
...
class InsertNodeCommand(QUndoCommand):
    ...
    def __init__(self, context, *args, **kwargs):
        ...
        self.node = self.context.m_scene.nodeFactory()#将节点之间用线连接
        self.node.x, self.node.y = self.context.m_pos
        self.node.num = len(self.context.m_scene.NodeList) + 1
        if self.context.m_activateNode.m_level == MainThemeLevel:#判断节点是否为中心主题
            self.node.setNodeLevel(SecondThemeLevel)#为中心主题则插入分支主题
        else:
            self.node.setNodeLevel(ThirdThemeLevel)#反之插入子主题
...
class TextFontCommand(QUndoCommand):
    """改变字体样式"""
    def __init__(self, context, *args, **kwargs):
        super(TextFontCommand, self).__init__(*args, **kwargs)
        self.context = context
        self.textFont = self.context.m_activateNode.m_textFont

    def undo(self):
        self.context.m_activateNode.setTextFont(self.textFont)
        self.context.m_scene.setActivateNode(self.context.m_activateNode)

    def redo(self):
        self.context.m_activateNode.setTextFont(self.context.m_textFont)
        self.context.m_scene.setActivateNode(self.context.m_activateNode)
